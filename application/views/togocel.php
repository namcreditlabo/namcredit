...
<head>
  ...
  <title >Transfert Togocel</title>
</head>

<body style="background-color:#EBEBEB">
  <div id="container">
<div><center> <h2 style="color : white"> SERVEZ-VOUS...  NAMCREDIT</h2> </center> </div>
       ...
       <div id="imgb">
          <!-- Start Formoid form-->
          <form action="traiter_togocel" id="formoid" style="background-color:#FFFFFF;" title="Formulaire d'envoie de crédit" method="post">
            <a href="<?php echo site_url('..\achat\togocel') ?> "><?php echo img('togocel.jpg','Recharger crédit Togocel') ; ?></a></br> </br> 

          <div><h2 class="title">Envoi de Crédit</h2></div>
          <div>
            <label class="title">Votre nom</label>
            <input type="text" name="nom" id="nom" value=""  placeholder="Un message sera envoyé au bénéficiaire en ce nom" required>
          </div>
          <div>
...
function numvalid(){
var expression = /^9[0-3]([-. ]?[0-9]{2}){3}$/; 
if(expression.test(document.getElementById('num').value)==false)

	alert('Numéro invalide (Enlevé le 228 ou vérifier que c\'est un numéro TOGOCEL )') ;
}
</script>
<script type="text/javascript">window.location.hash = '#image-3'</script>
